...
%% add montage plots for symptom scores and sig components
%plot worry, and social disinterest (moodiness had no surviving components)
sigcomps_worry = [1,2,3,4,6,11,12,15,16,17,24,32,36,37,43,45,47,48]; 
files = cell(1, numel(sigcomps_worry));
for i = 1:numel(sigcomps_worry)
    files{i} = sprintf('Depression_ICA_Output_Rest_Clinical_Outcomes_Results_group_component_ica_.nii,%d', comps(i));
end
...
title ('Significant Loading Components for Worry & Rumination: FDR Corrected '); 
set(gca, 'FontSize', 13);

% Save the figure
savefig('AllComponents_Worry_Rum_FDR_Corrected.fig');

%% next, plot for social disinterest
sigcomps_socialdis = [2 12 24 30 48 57]; 
files = cell(1, numel(sigcomps_socialdis));
for i = 1:numel(sigcomps_socialdis)
    files{i} = sprintf('Depression_ICA_Output_Rest_Clinical_Outcomes_Results_group_component_ica_.nii,%d', comps(i));
end
...
title ('Significant Loading Components for Social Disinterest: FDR Corrected '); 
set(gca, 'FontSize', 13);

% Save the figure
savefig('AllComponents_SocialDis_FDR_Corrected.fig');