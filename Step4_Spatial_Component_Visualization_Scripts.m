...
%all 44 components plotted together
...
%first positive
% Build file list
files = cell(1, numel(sigcompspos));
for i = 1:numel(sigcompspos)
    files{i} = sprintf('Depression_ICA_Output_Rest_Clinical_Outcomes_Results_group_component_ica_.nii,%d', comps(i));
end
...
title ('36 Significant Positive Components : FDR Corrected Plotted '); 
set(gca, 'FontSize', 13);

% Save the figure
savefig('AllComponents_Positive_FDR_Corrected.fig');

%% now plot negative components
% Build file list
files = cell(1, numel(sigcompsneg));
for i = 1:numel(sigcompsneg)
    files{i} = sprintf('Depression_ICA_Output_Rest_Clinical_Outcomes_Results_group_component_ica_.nii,%d', comps(i));
end
...
title ('32 Significant Negative Components : FDR Corrected Plotted '); 
set(gca, 'FontSize', 13);
